...
void send_web_menu(struct tcp_pcb *pcb) {
    ...
    snprintf(buf, sizeof buf,
        ...
        "<div class='status'>SD Card: <span class='%s'>%s</span></div>"
        "</div>",
    ...
    http_write_str(pcb,
        "<div class='menu-item'>"
        "<h3>WARNING: Backup & Restore (4MB Flash)</h3>"
        "<p style='color:#c22;font-weight:bold'>IMPORTANT: Click button ONCE and wait. Do NOT refresh!</p>"
        "<a class='btn' href='/action?cmd=backup_flash'>Backup Flash to SD</a>"
        "<a class='btn btn-danger' href='/action?cmd=restore_flash' onclick='return confirm(\"OVERWRITE flash chip?\")'>Restore Flash from SD</a>"
        "<p style='font-size:12px;color:#666;margin:8px 0'>Backup: ~60 seconds | Restore: ~2-3 minutes (DESTRUCTIVE!)</p>"
        "</div>"
        "<div class='menu-item'>"
...
void send_action_result_page(struct tcp_pcb *pcb, const char *cmd) {
    ...
    http_write_str(pcb,
        ...
        ".warning{background:#fff3cd;border:2px solid #ffc107;padding:15px;margin:15px 0;border-radius:8px;font-weight:bold}"
        "</style>"
        "</head><body>"
        "<h2>Action Complete</h2>"
        "<div class='warning'>WARNING: DO NOT REFRESH OR GO BACK! Operation completed successfully.</div>"
        "<pre>");
    ...
    http_write_str(pcb, "<p style='margin-top:20px'><a href='/menu' style='padding:10px 20px;background:#0a6;color:#fff;text-decoration:none;border-radius:6px'>Back to Menu</a></p>");
