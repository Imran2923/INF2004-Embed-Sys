...
void action_test_connection(void) {
    ...
    // Erase sector
    printf("Erasing 4K @0x%06X...\r\n", base_addr);
    uint8_t sr1 = 0;
    (void)timed_erase_4k(base_addr, &sr1);

    // Create a page with our message at start
    uint8_t page[256];
    ...
    (void)timed_prog_256(page_addr, page, &verr, &sr1);

    // Read back first 32 bytes
    uint8_t rb[32]={0};
    read_data(page_addr, rb, sizeof(rb));
    printf("Read-back (32B): ");
    for (int i=0;i<32;i++) printf("%02X ", rb[i]);
    printf("\r\nVerification %s (errors=%u). SR1=%02X\r\n",
           verr ? "FAILED" : "PASSED", verr, sr1);
